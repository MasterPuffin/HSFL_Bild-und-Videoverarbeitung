...
a = zeros;b = zeros;c = zeros; %Speicher reservieren
...
for i=1:count(1) %(forschleife durchlaufen)
    ...
    if ( Area > 3000 ) % !kleine Objekte nicht betrachten!
        ...
        sm(counter).Eccentricity = s(i).Eccentricity; %Wert für ConvexArea kopieren
        b(counter) = s(i).Eccentricity; %Konvexe Fläche für plot in einen Vektor schreiben
...
figure(2); plot(a, b, 'ro'); xlabel('Area'); ylabel('Eccentricity'); %2D Graph mit Area und ConvexArea
figure(3); scatter3(a,b,c); xlabel('Area'); ylabel('Eccentricity'); zlabel('Solidity'); %3D Graph mit allen Features
